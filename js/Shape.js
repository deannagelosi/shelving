class Shape {
    constructor() {
        this.inputGrid = [[]]; // (high res) user input grid

        // shape grids are stored as 2D arrays of booleans
        // - occupied squares as true, unoccupied as false
        this.shape = [[]]; // (high res) trimmed input grid
        this.lowResShape = [[]]; // 1/4th the width and height as shape, scaled to 1/4th the resolution
        this.bufferShape = [[]]; // low res + one extra square on right and top
        this.bufferHeight;
        this.bufferWidth;
        this.title;
    }
    ...
    saveUserInput(_title, _inputGrid) {
        ...
        let paddedShape = this.shape.map(row => [...row]);
        ...
        this.bufferShape = [];
        for (let y = 0; y < this.lowResShape.length; y++) {
            let newRow = [...this.lowResShape[y]];
